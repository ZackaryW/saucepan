...
When more than one registered index describes the same target, Saucepan SHALL resolve using registration order and SHALL report that a later index also described the target.
...
- **THEN** the earlier-registered index supplies the manifest and Saucepan reports the shadowed entry
