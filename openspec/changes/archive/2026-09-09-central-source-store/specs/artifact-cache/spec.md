...
Every retained Git snapshot SHALL be a ZIP of the complete exported source tree, with paths relative to its repository root and without `.git` directories or gitfiles at any depth. Normal exported files such as `.gitignore` SHALL be preserved. Snapshot identity SHALL account for source identity, revision, export behavior, and included dependencies. A requested subdirectory SHALL identify content to extract from that snapshot, not a separate snapshot unit.
