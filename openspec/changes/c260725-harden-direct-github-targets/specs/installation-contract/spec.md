...
Saucepan SHALL use `sauce.name` from the manifest as the installed identity for lookup, update, path, cat, and uninstall operations. The source target SHALL be retained as provenance rather than used as the installed identity.
...
Saucepan SHALL map known missing targets or manifests to exit code 1, Git/gh transport or execution failures to exit code 2, missing or invalid source configuration to exit code 3, origin conflicts to exit code 4, and unexpected failures to exit code 5.
...
#### Scenario: Configured manifest is absent
- **WHEN** a fetched target does not contain the configured root manifest
